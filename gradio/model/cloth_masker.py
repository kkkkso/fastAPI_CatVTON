...
def save_mask(mask: np.ndarray, file_path: str):
    ...
    mask_image = Image.fromarray(mask.astype(np.uint8) * 255)  # 이진 마스크인 경우 0과 1을 0과 255로 변환
...
class AutoMasker:
    ...
    def cloth_agnostic_mask(
        ...
        **kwargs
        ...
        #save_mask(densepose_mask, 'densepose_mask.png') #추가
        # 추추추추가
        # new_area = part_mask_of(['torso', 'big arms'], densepose_mask, DENSE_INDEX_MAP)
        # save_mask(new_area, 'new_mask.png')  

        schp_lip_mask = np.array(schp_lip_mask)
        #save_mask(schp_lip_mask, 'schp_lip_mask.png')
        schp_atr_mask = np.array(schp_atr_mask)
        #save_mask(schp_atr_mask, 'schp_atr_mask.png')
        ...
        # mask_area = hull_mask(mask_area * 255) // 255  # Convex Hull to expand the mask area / 주석처리 하였다.
